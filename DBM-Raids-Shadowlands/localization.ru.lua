...
L:SetOptionLocalization({
	timerContainers2			= "Показывать таймер, который будет показывать прогресс заполнения контейнера и время, оставшееся до заполнения"
})
...
L:SetOptionLocalization({
	PairingBehavior		= "Установить режим мода для $spell:361989. Используется предпочтение рейд-лидеров, если они используют DBM",
